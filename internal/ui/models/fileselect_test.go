...
func TestInputModeNavigationKeyConflicts(t *testing.T) {
	...
	for _, letter := range testLetters {
		initialLength := len(model.inputText)
		
		// Type the letter
		model, _ = model.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{letter}})
		
		// Check if letter was added to input
		if len(model.inputText) != initialLength+1 {
			t.Errorf("Letter '%c' was not added to input. Expected length %d, got %d. Input: '%s'", 
				letter, initialLength+1, len(model.inputText), model.inputText)
		}
		
		// Verify the letter is actually the one we typed
		if len(model.inputText) > 0 && model.inputText[len(model.inputText)-1] != byte(letter) {
			t.Errorf("Last character should be '%c', but got '%c'", 
				letter, model.inputText[len(model.inputText)-1])
		}
	...
	if model.inputText != expectedInput {
		t.Errorf("Expected input text '%s', got '%s'", expectedInput, model.inputText)
	}
}
...
func TestInputModeEscapeBehavior(t *testing.T) {
	...
	if model.inputText != testText {
		t.Errorf("Expected input text '%s', got '%s'", testText, model.inputText)
	}
	...
	if model.inputText != "" {
		t.Errorf("Input text should be cleared on Esc, got '%s'", model.inputText)
	}
}
